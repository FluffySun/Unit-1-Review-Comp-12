...
void show() {
  ...
  if (img!=null) {
    ...
    //this condition is wrong? exclusive
    //messes with other pre-existing functions
    drawImage(); //"lolirock.png"
  } else {
    drawRect();
    drawLabel();
  }
...
void drawImage() { //is it loadImage?
  imageMode (CENTER);
  
  //no need? down
  if (mouseX > x - w/2 && mouseX < x + w/2 && mouseY > y - h/2 && mouseY < y + h/2) {
    color(highlight); //mouse touch effect
  } else {
    color (normal);
  }
  //no need? up^
